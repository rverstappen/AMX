MODULE_NAME='AvMatrixAutoPatchDsp' (char configFile[])


DEFINE_VARIABLE

constant char DBG_MODULE[] = 'AutoPatch-DSP'
...
DEFINE_FUNCTION handleMatrixSwitch (AvMatrixMapping mappings[])
{
	...
	{
	    sendString (DBG_MODULE, gGeneral.mDevSwitch, "'CI',itoa(mappings[i].mInput),'O',itoa(mappings[i].mOutputs[o]),'T'")
	}
    }
}
...
DEFINE_FUNCTION handleMatrixStatusRequest ()
{
    ...
    {
	sendString (DBG_MODULE, gGeneral.mDevSwitch, "'SL0I',itoa(input),'T'")
    }
}
