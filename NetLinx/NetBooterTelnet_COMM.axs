...
DEFINE_FUNCTION handleHttpResponse (integer netBooterId, char msg[])
{
    ...
    select
    {
    active (isValidStatus(msg)):
    {
	// Received a status update.
	handlePowerStatus(netBooterId, msg)
    }
    active (find_string(msg,'$A0,',1)): // comma in message
    {
	// Received an old-style status update.
	integer len
...
DEFINE_FUNCTION handlePowerStatus (integer netBooterId, char msg[])
{
    integer portId, len
    ...
    for (portId = 1; (portId <= gNetBooters[netBooterId].mNumPorts) && (len > 0); portId++, len--)
    {
...
DEFINE_FUNCTION integer isValidStatus(char msg[])
{
